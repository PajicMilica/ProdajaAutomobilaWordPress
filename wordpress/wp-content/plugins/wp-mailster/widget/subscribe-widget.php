...
class MstSubscribe_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
        ...
        $headerTxt = apply_filters( 'Mailing List Subscription', $instance['title'] );
        ...
        $formSessionInfo->wp_widget_id = $args['widget_id'];
        if($formSessionInfo->wp_widget_id && ( strlen('mstsubscribe_widget-') < strlen($formSessionInfo->wp_widget_id) ) ){
            $formSessionInfo->wp_widget_id_key_only = substr($formSessionInfo->wp_widget_id, strlen('mstsubscribe_widget-'));
        }else{
            $formSessionInfo->wp_widget_id_key_only = -1; // not found
        }
        ...
        if(version_compare(phpversion(), "5.4.0", ">=")){
            if(session_status() == PHP_SESSION_NONE){
                $log->debug('No session detected, start new session (PHP: '.phpversion().')');
                session_start();
            }
        }else{
            if(session_id() === ''){
        ...
        if(array_key_exists('wpmst_subscribe_forms', $_SESSION)){
            $log->debug('subscribe-widget Already '.count($_SESSION['wpmst_subscribe_forms']).' forms in session');
            $subscribeFormsInSession = $_SESSION['wpmst_subscribe_forms'];
        }else{
            $log->debug('subscribe-widget No forms in session, yet');
            $subscribeFormsInSession = array();
        }
        ...
        //if( !is_admin() ) $log->debug('subscribe-widget SESSION after adding formId '.$formIdentifier.': '.print_r($_SESSION['wpmst_subscribe_forms'], true));
