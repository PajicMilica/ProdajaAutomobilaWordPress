...
function mailster_install_or_update() {
    ...
    if(!get_option('wpmst_cfg_version_license')){
        $log->info('mailster_install_or_update No license info found, init!', MstConsts::LOGENTRY_INSTALLER);
        add_option('wpmst_cfg_allow_send', true);
        add_option('wpmst_cfg_version_license', 'free');
        add_option('wpmst_cfg_current_version', 'free');
    }

}

function setupInitialDatabaseIfNeeded(){
    $log = MstFactory::getLogger();
    $dbUtils = MstFactory::getDBUtils();
...
function wpmst_update_db_check(){
    ...
    if($legacyDbOption !== '47.11'
        ...
        if($legacyDbOption !== '47.11'){
            ...
            $legacyOptions = array();
            $legacyOptions[] = array('mailster_db_version', '1.6.16.0');
        ...
        if(version_compare($dbVersion, "1.6.16", "<")){ // dummy DB update
            $log->info('wpmst_update_db_check Update db version to: 1.6.16', MstConsts::LOGENTRY_INSTALLER);
            update_option('wpmst_cfg_mailster_db_version', '1.6.16');
        }

        //$log->info('wpmst_update_db_check Update db version to: '.$currentVersion, MstConsts::LOGENTRY_INSTALLER);
