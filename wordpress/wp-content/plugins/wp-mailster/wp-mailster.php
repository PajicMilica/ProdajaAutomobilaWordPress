...
class wpmst_mailster {
	...
	public function wpmst_mailster_menu() {
		...
		$hook = add_submenu_page('wpmst_mailster_intro', __('Add Mailing List','wp-mailster'), __('Add Mailing List', 'wp-mailster'), $admin_level, 'mst_mailing_list_add',array($this,'mst_mailing_list_add'));
		$hook = add_submenu_page('wpmst_mailster_intro', __('Manage Recipients','wp-mailster'), __('Manage Recipients', 'wp-mailster'), $admin_level, 'mst_recipient_management',array($this,'mst_recipient_management'));
		$hook = add_submenu_page('wpmst_mailster_intro', __('Add Recipient','wp-mailster'), __('Add Recipient', 'wp-mailster'), $admin_level, 'mst_list_members_add',array($this,'mst_list_members_add'));
		$hook = add_submenu_page('wpmst_mailster_intro', __('Add Group','wp-mailster'), __('Add Group', 'wp-mailster'), $admin_level, 'mst_list_groups_add',array($this,'mst_list_groups_add'));
		...
		$hook = add_submenu_page('wpmst_mailster_intro', __('Add Server','wp-mailster'), __('Add Server', 'wp-mailster'), $admin_level, 'mst_servers_add',array($this,'mst_servers_add'));
		...
		$hook = add_submenu_page('wpmst_mailster_intro', __('Add User','wp-mailster'), __('Add User', 'wp-mailster'), $admin_level, 'mst_users_add',array($this,'mst_users_add'));
		...
		$hook = add_submenu_page('wpmst_mailster_intro', __('Add Group','wp-mailster'), __('Add Group', 'wp-mailster'), $admin_level, 'mst_groups_add', array($this,'mst_groups_add'));
		// mail archive
...
\*********************************************/
	...
	function mst_display_select_field($title, $name, $options = array(), $value, $placeholder = null, $required = false, $info = null, $readonlyOptions = array(), $disabled = false, $embedHtml = false, $outerClasses = false, $selectClasses = false, $multiSelect = false) {
        ?>
		<tr class="<?php if($required) { echo 'form-required '; } if($outerClasses) { echo $outerClasses; } ?>" >
			<th scope="row">
				<label for="<?php echo $name; ?>"><?php echo $title; ?> </label>
    ...
    public function wpmst_subscribe_plugin(){
        ...
        $log->debug('wpmst_subscribe_plugin Have '.count($_SESSION['wpmst_subscribe_forms']).' forms in session');
        ...
        if($subscribeFormsInSession && is_array($subscribeFormsInSession)){
            foreach($subscribeFormsInSession AS $sessionKeyIndex => $formInSession){
                if(property_exists($formInSession, 'id') && $formInSession->id === $formId){
                    ...
                    if(property_exists($formInSession, 'origin') && $formInSession->origin == 'widget' && property_exists($formInSession, 'wp_widget_id_key_only')){
                        $mstSubscribeWidgetsOptions = get_option('widget_' . 'mstsubscribe_widget');
                        if($mstSubscribeWidgetsOptions && array_key_exists($formInSession->wp_widget_id_key_only, $mstSubscribeWidgetsOptions)){
                            $thisWidgetsSettings = $mstSubscribeWidgetsOptions[$formInSession->wp_widget_id_key_only];
                            $log->debug('wpmst_subscribe_plugin belongs to widget with settings: '.print_r($thisWidgetsSettings, true));
                            $add2Group = property_exists($formInSession, 'subscribeAdd2Group') ? $formInSession->subscribeAdd2Group : $thisWidgetsSettings['subscription_group_choice'];
                        }
        ...
        if($foundInSession){
            ...
            if($res && $foundSessionKeyIndex >= 0){
                ...
                $log->debug('subscribe-widget Now '.count($_SESSION['wpmst_subscribe_forms']).' forms in session');
            }
    ...
    public function wpmst_unsubscribe_plugin(){
        ...
        $log->debug('wpmst_unsubscribe_plugin Have '.count($_SESSION['wpmst_subscribe_forms']).' forms in session');
        ...
        $noListChosen = array('id'=>'no_list', 'msg'=>__( 'You have no mailing list chosen', 'wp-mailster' ));
        $unsubscribeIncative = array('id'=>'unsub_inactive', 'msg'=>__( 'Unsubscribing currently not possible', 'wp-mailster' ));
        $notSubscribed = array('id'=>'unsub_inactive', 'msg'=>__( 'Email address is not subscribed', 'wp-mailster' ));
        $captchaCodeWrong = array('id'=>'captcha_wrong', 'msg'=>__( 'The captcha code you entered was wrong, please try again', 'wp-mailster' ));
...
add_action( 'wp_ajax_saveLicChkResult', 'saveLicChkResult_callback' );
function saveLicChkResult_callback() {
    $log = MstFactory::getLogger();
    $mstUtils = MstFactory::getUtils();
    $resultArray = array();
    $res = __( 'Reset timer called', 'wp-mailster' );
    ...
    $ajaxParams = $mstUtils->jsonDecode(stripslashes($ajaxParams));
    $task = $ajaxParams->{'task'};
    $log->debug('saveLicChkResult_callback params: '.print_r($ajaxParams, true));
...
/*  */
	...
	function wpmst_admin_notice() {
		if(get_option('wpmst_cfg_version_license') == 'free' && get_option('wpmst_cfg_current_version') != 'free') {
			?>
			<div class="notice notice-warning is-dismissible wpmst-license-msg">
				<p><?php _e( 'Your WP Mailster version will become the FREE edition the next time you update it. Go to <a href="admin.php?page=wpmst_settings">WPMailster settings</a> to add your license key', 'wp-mailster' ); ?></p>
			</div>
			<?php 
			...
			<script type="text/javascript">
				...
				jQuery(document).ready(function() {
					reEvaluateLic(<?php echo json_encode(trim(get_option('wpmst_cfg_license_key'))); ?>, <?php echo json_encode($mstversion); ?>, <?php echo json_encode($wp_version); ?>, callBackLicEval);
				});
			</script>
			<?php
		}
...
/*  */
	...
	function wpmst_double_opt_check() {
        ...
        if(isset($_GET['confirm_unsubscribe']) && $_GET['confirm_unsubscribe'] == "indeed2") {
            ...
            $hash = sanitize_text_field($_SESSION['hash']);
    ...
    add_action( 'wp_loaded', 'wpmst_moderation_decision' );
    function wpmst_moderation_decision() {
        ...
        if($mail){
            ...
            $hashOk = ($saltedKeyHash==$hash);
            $_SESSION['wpmst_mod_decision_mail_id'] = $mailId;
            $_SESSION['wpmst_mod_decision_hash'] = $hash;
            $_SESSION['wpmst_mod_decision_salt'] = $salt;
            $_SESSION['wpmst_mod_decision_list_id'] = $listId;
            if($hashOk){
            ...
            $_SESSION['wpmst_mod_decision_result_obj'] = $res;
        }else{
        ...
        if($doForward){
            header("HTTP/1.1 301 Moved Permanently");
            header("Status: 301 Moved Permanently");
            header("Location: " . plugins_url() . "/wp-mailster/view/moderation/".$targetPage.".php?success=" . $successful);
            header("Connection: close");
            exit(0);
        }
    }
