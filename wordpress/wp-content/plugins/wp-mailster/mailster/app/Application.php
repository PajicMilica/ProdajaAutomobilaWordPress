...
if (preg_match('#' . basename(__FILE__) . '#', $_SERVER['PHP_SELF'])) {
	...
	class MstApplication
	{
		...
		public static function detectSystemProblems(){
			...
			$wpVersionOk		= version_compare($currentVersion,'3.4.0'); // at least WordPress 3.4.0
			...
			if ( $wpVersionOk < 0 ) {
				...
				$res->error = true;
				$res->errorMsg = __("parse_ini_file PHP function is disabled, please check system requirements", 'wp-mailster');
		...
		public static function performMaintenance($minDuration=0, $execEnd=0){
            ...
            foreach($lists AS $list){
                if($list->archive_offline > 0){
                    if(($execEnd > 0) && (($execEnd - time()) <= $minDuration)){
                        $log->debug('Timeout, do not work on offlineF archiving');
                        break;
                    }
