...
if (preg_match('#' . basename(__FILE__) . '#', $_SERVER['PHP_SELF'])) {
	...
	class MstLog
	{
	    ...
	    public static function log($msg, $level='INFO', $typeNr=0)
	    {
            ...
            if((array_key_exists('action', $_POST) && $_POST['action'] == 'heartbeat') && $level == self::DEBUG){
                return; // do not log anything DEBUG during heartbeat requests!
            }
			if(array_key_exists('REQUEST_URI', $_SERVER) && (strpos($_SERVER['REQUEST_URI'], 'wp-json/wp/v2') !== false) && $level == self::DEBUG){
				return true; // do not log anything DEBUG during REST API requests!
			}
