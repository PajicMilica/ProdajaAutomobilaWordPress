<?php
	...
	if(isset($_POST['task']) && $_POST['task'] == "startimport") {
		...
		if ( isset( $_POST['autoformat'] ) ) {
			$autoFormat = intval( $_POST['autoformat'] );
		} else {
			$autoFormat = 0;
		}
							...
							if(!$users){
								...
								<tr>
									<td width="100px" style="text-align:right;" colspan="2">
										<label for="autoformat" title="<?php echo _e( 'Automatically detect CSV file format', 'wp-mailster' ); ?>">
											<?php echo _e( 'Automatically detect CSV file format', 'wp-mailster' ); ?>
										</label>
									</td>
									<td width="200px">
