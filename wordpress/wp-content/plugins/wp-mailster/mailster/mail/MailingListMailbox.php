...
class MstMailingListMailbox
{
	...
	function getMessage($nr){					
        ...
        $mail->receive_timestamp	= $dbUtils->getTimestampFromDate(date("Y-m-d H:i:s",$header['udate']));
        $log->debug('From udate '.$header['udate'].' to UTC date '.date("Y-m-d H:i:s",$header['udate']).' to MySQL localized '.$mail->receive_timestamp);
    ...
    function isMailToBeModerated($senderInfo){
        ...
        if($moderationNeeded){
            if($this->mList->mod_approve_recipients > 0){
            ...
            if($this->mList->mod_approve_group > 0){
