...
jQuery(document).ready(function() {
	...
	jQuery(document).ready(function($) {
		$('#multiselect').multiselect({
			search: {
				left: '<div class="ms2side__header"><input type="text" name="q" class="form-control" placeholder="Search..." /></div>',
				right: '<div class="ms2side__header"><input type="text" name="q" class="form-control" placeholder="Search..." /></div>'
			},
...
function reEvaluateLic(license_key, mstversion, wpversion, callBack){
...
function getLicSubInfo(license_key, mstversion, wpversion, date_format, callBack){
    var data2send = '{ "key": "' + license_key + '", "url": "' + window.location.protocol + "//" + window.location.host + '", "wpversion": "' + wpversion + '", "mstversion": "' + mstversion + '", "date_format": "' + date_format + '" }';
    var data = {
        'mstAjaxData' : data2send
    };
    var url = "https://wpmailster.com/subinfo";
    jQuery.post(url, data, function(response){callBack(response)}, 'json');
}
